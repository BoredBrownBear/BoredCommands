...
public class Config {
    ...
    public Config(String name) {
        this.name = name;
        this.configFile = new File(FabricLoader.getInstance().getConfigDir().toString() + "/" + name + ".json");
            ...
            try(FileWriter writer = new FileWriter(configFile)) {
                for (Map.Entry<String, Object> property : properties.entrySet()) {
                    if (property.getValue() instanceof JsonElement) {
                        ...
                        BoredCommands.LOGGER.error("Unable to parse json config property! File: " + name + ".json with Property: " + property.getKey());
                        continue;
                    }
                }
                ...
                BoredCommands.LOGGER.fatal("You done borked something with your config", e);
            }
        }
    }
    /**
    ...
    public boolean getBoolean(String key) {
        return this.config.has(key) && this.config.get(key).getAsBoolean();
    }
    ...
    private void loadConfig() {
        if(!configFile.exists()) {
            BoredCommands.LOGGER.error("Unable to load config file " + name + ".json, File not found!");
            return;
        }

        try {
            BoredCommands.LOGGER.info("Loading config file " + name + ".json");
    ...
    private void saveConfig(FileWriter writer) {
        BoredCommands.LOGGER.info("Creating config file " + name + ".json");
